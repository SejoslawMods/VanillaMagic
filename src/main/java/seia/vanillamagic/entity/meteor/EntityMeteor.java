...
import seia.vanillamagic.util.explosion.ExplosionUtil;
...
public class EntityMeteor extends EntityLargeFireball {
	public EntityMeteor(EntityLivingBase castingEntity, double spawnMeteorX, double spawnMeteorY, double spawnMeteorZ,
		...
		setSize(VMConfig.BASIC_METEOR_SIZE, VMConfig.BASIC_METEOR_SIZE);

		this.explosionPower = VMConfig.BASIC_METEOR_EXPLOSION_POWER;
		this.motionX = 0.0D;
		this.motionY = 0.0D;
		this.motionZ = 0.0D;
	...
	protected void onImpact(RayTraceResult result) {
		...
		if (!this.world.isRemote) {
			...
			ExplosionUtil.newExplosion2(this.world, (Entity) null, this.posX, this.posY, this.posZ,
					(float) this.explosionPower, flag, flag);
			this.setDead();
		}
	}
