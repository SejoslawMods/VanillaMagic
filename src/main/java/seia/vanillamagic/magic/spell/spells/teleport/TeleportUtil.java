...
public final class TeleportUtil {
	...
	public static Entity entityChangeDimension(Entity entity, int newDimId) {
		if (!EventUtil.postEvent(new EventTeleportEntity.ChangeDimension(entity, entity.getPosition(), newDimId))) {
	...
	public static void teleportEntity(Entity entityToBeTeleport, BlockPos teleportTo) {
		if (!EventUtil.postEvent(new EventTeleportEntity(entityToBeTeleport, teleportTo))) {
	...
	public static void changePlayerDimensionWithoutPortal(EntityPlayerMP player, int dimension, BlockPos pos) {
		if (EventUtil.postEvent(new EventTeleportEntity.ChangeDimension(player, pos, dimension))) {
			return;
		}
	...
	/**
	 * It will teleport You to: CoordX = player.posX etc.
	 */
	...
	public static Entity changeDimension(Entity entity, int newDimId, BlockPos newPos) {
		if (entity.world.isRemote || entity.isDead || !ForgeHooks.onTravelToDimension(entity, newDimId)) {
			return null;
		}
		...
		if (newDimId == 1) {
			...
			if (newDimId == -1) {
				d0 = MathHelper.clamp(d0 / 8.0D, worldServerNew.getWorldBorder().minX() + 16.0D,
						worldServerNew.getWorldBorder().maxX() - 16.0D);
				d1 = MathHelper.clamp(d1 / 8.0D, worldServerNew.getWorldBorder().minZ() + 16.0D,
						worldServerNew.getWorldBorder().maxZ() - 16.0D);
			} else if (newDimId == 0) {
				d0 = MathHelper.clamp(d0 * 8.0D, worldServerNew.getWorldBorder().minX() + 16.0D,
						worldServerNew.getWorldBorder().maxX() - 16.0D);
				d1 = MathHelper.clamp(d1 * 8.0D, worldServerNew.getWorldBorder().minZ() + 16.0D,
	...
	private static void transferEntityToWorld(Entity entityIn, int lastDimension, WorldServer oldWorldIn,
			WorldServer toWorldIn, VMTeleporter teleporter) {
		...
		if (false && entityIn.dimension == -1) {
			posX = MathHelper.clamp(posX / 8.0D, toWorldIn.getWorldBorder().minX() + 16.0D,
					toWorldIn.getWorldBorder().maxX() - 16.0D);
			posZ = MathHelper.clamp(posZ / 8.0D, toWorldIn.getWorldBorder().minZ() + 16.0D,
	...
	private static void updateTimeAndWeatherForPlayer(EntityPlayerMP player, WorldServer worldIn, int dimension) {
		...
		player.connection.sendPacket(new SPacketTimeUpdate(worldIn.getTotalWorldTime(), worldIn.getWorldTime(),
