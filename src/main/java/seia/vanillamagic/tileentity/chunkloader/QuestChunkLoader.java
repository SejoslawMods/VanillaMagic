...
public class QuestChunkLoader extends Quest {
	...
	public void chunkLoaderPlaced(PlaceEvent event) {
		...
		if (ItemStackUtil.isNullStack(itemInHand) || (itemInHand.getItem() == null)
				|| !Block.isEqualTo(Block.getBlockFromItem(itemInHand.getItem()), Blocks.ENCHANTING_TABLE)) {
			return;
		}
	...
	public static boolean isChunkLoaderBuildCorrectly(World world, BlockPos chunkLoaderPos) {
		...
		if (Block.isEqualTo(world.getBlockState(obsidianUnder).getBlock(), Blocks.OBSIDIAN)
				&& Block.isEqualTo(world.getBlockState(obsidianTop).getBlock(), Blocks.OBSIDIAN)
				&& Block.isEqualTo(world.getBlockState(obsidianLeft).getBlock(), Blocks.OBSIDIAN)
				&& Block.isEqualTo(world.getBlockState(obsidianRight).getBlock(), Blocks.OBSIDIAN)
				&& Block.isEqualTo(world.getBlockState(obsidianBottom).getBlock(), Blocks.OBSIDIAN)) {
			return true;
		}
