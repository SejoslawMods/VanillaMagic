...
public class NBTUtil
{
	...
	{
		if (nbt.hasKey(NBT_UNLOCALIZED_NAME_BLOCK)) return nbt.getString(NBT_UNLOCALIZED_NAME_BLOCK);
		return "";
	}
	...
	{
		if ((inv == null) || (nbt == null)) return null;
		...
		for (int i = 0; i < nbtItemsList.tagCount(); i++)
		{
			...
			if (gettedByte >= 0 && gettedByte < inv.getSizeInventory())
				inv.setInventorySlotContents(gettedByte, ItemStackUtil.loadItemStackFromNBT(nbtItemTagCompound));
		}
		return inv;
	}
	...
	{
		if (nbt.hasKey(NBT_SERIALIZABLE)) nbtSerial.deserializeNBT(nbt.getCompoundTag(NBT_SERIALIZABLE));
		return nbtSerial;
	}
	...
	{
		if ((handler == null) || (nbt == null)) return null;
	...
	{
		if (tag == null || !tag.hasKey(key)) return new NBTTagCompound();
