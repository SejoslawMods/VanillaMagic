...
import com.github.sejoslaw.vanillamagic.tileentity.machine.farm.TreeHarvestUtils;
...
public class FarmerTree implements IFarmer {
	...
	protected boolean canPlant(World worldObj, BlockPos pos) {
		...
		return (sapling.canPlaceBlockAt(worldObj, pos)
				&& ground.canSustainPlant(bs, worldObj, grnPos, Direction.UP, plantable));
	}
	...
	public IHarvestResult harvestBlock(TileFarm farm, BlockPos pos, Block block, IBlockState state) {
		...
		for (int i = 0; i < res.harvestedBlocks.size() && hasAxe; ++i) {
			...
			if (blk instanceof IShearable && hasShears
				...
				chance = ForgeEventFactory.fireBlockHarvesting(drops, worldObj, coord, farm.getBlockState(coord),
						fortune, chance, false, null/* fakePlayer */);
				wasAxed = true;
			}
			...
			if (drops != null) {
				for (ItemStack drop : drops) {
					if (worldObj.rand.nextFloat() <= chance) {
						res.drops.add(new ItemEntity(worldObj, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
								drop.copy()));
					}
				}
			}
