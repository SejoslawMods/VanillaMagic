...
import com.github.sejoslaw.vanillamagic.tileentity.machine.farm.HarvestResult;
...
public class FarmerPlantable implements IFarmer {
	...
	public boolean prepareBlock(TileFarm farm, BlockPos bc, Block block, IBlockState meta) {
		...
		for (int i = 0; i < inv.getSizeInventory(); ++i) {
			...
			if ((!ItemStackUtil.isNullStack(invStack)) && (invStack.getItem() instanceof IPlantable)) {
				seedStack = invStack;
			}
		}
		...
		Block ground = farm.getBlock(bc.offset(Direction.DOWN));
	...
	protected boolean plant(TileFarm farm, World worldObj, BlockPos bc, ItemStack plantable) {
		if (ItemStackUtil.getStackSize(plantable) <= 0) {
			return false;
		}
	...
	protected boolean canPlant(World worldObj, BlockPos bc, ItemStack plantable) {
		...
		return (target != null && target.getBlock().canPlaceBlockAt(worldObj, bc) && ground.canSustainPlant(groundBS,
				worldObj, groundPos, Direction.UP, ((IPlantable) plantable.getItem())));
	}
	...
	public IHarvestResult harvestBlock(TileFarm farm, BlockPos bc, Block block, IBlockState meta) {
		if (!canHarvest(farm, bc, block, meta) || !farm.hasHoe()) {
			return null;
		}
		...
		float chance = ForgeEventFactory.fireBlockHarvesting(drops, worldObj, bc, meta, fortune, 1.0F, false, null);
		farm.damageHoe(1, bc);
				...
				if (stack != null && ItemStackUtil.getStackSize(stack) > 0 && worldObj.rand.nextFloat() <= chance) {
					if (!removed && isPlantableForBlock(stack, block)) {
						...
						if (ItemStackUtil.getStackSize(stack) > 0) {
							result.add(new ItemEntity(worldObj, bc.getX() + 0.5, bc.getY() + 0.5, bc.getZ() + 0.5,
									stack.copy()));
						}
					} else {
						result.add(new ItemEntity(worldObj, bc.getX() + 0.5, bc.getY() + 0.5, bc.getZ() + 0.5,
