...
public class GuiVMQuests extends GuiScreen implements IProgressMeter {
	...
	public GuiVMQuests(GuiScreen parentScreen, PlayerEntity playerWhoOpenedBook) {
		this.parentScreen = parentScreen;
		this.player = playerWhoOpenedBook;
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		if (!this.loadingQuests && (button.id == 1)) {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		if (this.loadingQuests) {
			...
			this.drawCenteredString(this.fontRenderer, "Loading Quests", this.width / 2, this.height / 2,
					Color.WHITE.getRGB());
		} else {
			if (Mouse.isButtonDown(0)) {
				if (this.scrolling == 0 || this.scrolling == 1) {
					if (this.scrolling == 0) {
						this.scrolling = 1;
					} else {
			...
			// Scrolling
			if (this.xScrollTarget < (double) X_MIN * X_MIN_WIDTH_MULTIPLIER) {
				this.xScrollTarget = (double) X_MIN * X_MIN_WIDTH_MULTIPLIER;
			}

			if (this.yScrollTarget < (double) Y_MIN * Y_MIN_HEIGHT_MULTIPLIER) {
				this.yScrollTarget = (double) Y_MIN * Y_MIN_HEIGHT_MULTIPLIER;
			}

			if (this.xScrollTarget >= (double) X_MAX / X_MAX_WIDTH_MULTIPLIER) {
				this.xScrollTarget = (double) (X_MAX - 1) / X_MAX_WIDTH_MULTIPLIER;
			}

			if (this.yScrollTarget >= Y_MAX_HEIGHT_NEEDED_PIXELS + (double) Y_MAX / Y_MAX_HEIGHT_MULTIPLIER) {
				this.yScrollTarget = Y_MAX_HEIGHT_NEEDED_PIXELS + (double) (Y_MAX - 1) / Y_MAX_HEIGHT_MULTIPLIER;
			}
