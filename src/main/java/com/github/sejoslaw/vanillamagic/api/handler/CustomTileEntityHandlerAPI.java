package com.github.sejoslaw.vanillamagic.api.handler;
...
/**
 * CustomTileEntityHandler is used to hold all {@link ICustomTileEntity}s. Also
 * it is use to Save / Load {@link ICustomTileEntity}s. <br>
 * <br>
 * To make Your CustomTile saving / loading properly You must register Your tile
 * using full class name:<br>
 * GameRegistry.registerTileEntity(MyCustomTile.class,
 * "my.package.with.tile.MyCustomTile");<br>
 * <br>
 * Adding / removing should be register after some action happened. See
 * description of methods for more information.
 *
...
public class CustomTileEntityHandlerAPI {
     ...
     * Handler class
     */
    private static final String HANDLER_CLASS = "com.github.sejoslaw.vanillamagic.handler.CustomTileEntityHandler";

    private CustomTileEntityHandlerAPI() {
    }

    /**
     * This method should be use to register {@link ICustomTileEntity} into the
     * CustomTileEntityHandler.<br>
     * For instance VanillaMagic CustomTiles are registered after clicked on block
     * (mainly).
     *
    ...
    public static boolean addCustomTileEntity(ICustomTileEntity customTileEntity, World world) {
        return addCustomTileEntity(customTileEntity, world.getDimension().getType().getId());
    }
     ...
     *
     * @param customTileEntity ICustomTileEntity which should be added
     * @param dimension        dimension to which this tile should be added. It
     *                         should be -
     *                         customTileEntity.getTileEntity().getWorld().provider.getDimension();
     *                         (Overworld is 0)
     * @return TRUE if the tile was added successfully
     */
    public static boolean addCustomTileEntity(ICustomTileEntity customTileEntity, int dimension) {
        try {
            ...
            VanillaMagicAPI.LOGGER.log(Level.ERROR, "Error while adding ICustomTileEntity to World. (Class: "
                    + customTileEntity.getClass().getName() + "), Dimension: " + dimension);
            e.printStackTrace();
            return false;
        }
    }
    ...
    /**
     * This method should be use to delete the {@link ICustomTileEntity} at the
     * specified position.<br>
     * For instance VanillaMagic CustomTiles are removed if the block was destroyed
     * (mainly).
     *
    ...
    public static boolean removeCustomTileEntityAtPos(World world, BlockPos pos) {
        try {
            ...
            VanillaMagicAPI.LOGGER.log(Level.ERROR,
                    "Error while removing CustomTileEntity at position" + pos.toString());
            e.printStackTrace();
            return false;
        }
    }
    ...
    /**
     * This method should be use to delete the {@link ICustomTileEntity} at the
     * specified position and inform Player about it.<br>
     * For instance VanillaMagic CustomTiles are removed if the block was destroyed
     * (mainly).
     *
     * @param world  world from which we will delete the tile
     * @param pos    position of the deleting tile
     * @param player player that should be informed about CustomTileEntity being
     *               removed
     */
    public static void removeCustomTileEntityAndSendInfoToPlayer(World world, BlockPos pos, PlayerEntity player) {
        try {
            Class<?> clazz = Class.forName(HANDLER_CLASS);
            Method method = clazz.getMethod("removeCustomTileEntityAndSendInfoToPlayer", World.class, BlockPos.class,
                    PlayerEntity.class);
    ...
    /**
     * Returns the {@link ICustomTileEntity} at the specified position from the
     * specified dimension.
     *
     * @param tilePos   position of the tile
     * @param dimension dimension in which the tile is (Overworld is 0)
     * @return the {@link ICustomTileEntity} at the specified position. NULL if
     * there is no tile at the given position.
     */
    @Nullable
