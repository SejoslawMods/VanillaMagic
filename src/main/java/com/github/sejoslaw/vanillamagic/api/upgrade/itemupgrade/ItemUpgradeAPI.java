...
public class ItemUpgradeAPI {
     ...
     */
    private static final String REGISTRY_CLASS = "com.github.sejoslaw.vanillamagic.item.itemupgrade.ItemUpgradeRegistry";
     ...
     *
     * @param mappingName   -> For instance: "_pickaxe" or "_sword" or "_axe" or
     *                      "_myNewMapping" itd.
     * @param clazz         -> MyUpgradeClass.class
     * @param localizedName -> If the mapping don't exists this localized name will
     *                      be use as a name for new mapping. For instance: for
     *                      "_pickaxe" localized name is "Pickaxe". It must be human
     *                      readable name.
     */
    ...
    /**
     * Add a single value to the mapping. If mapping doesn't exists it will add a
     * new mapping. <br>
    ...
    public static void addItemToMapping(String mappingName, Item item, String localizedName) {
        try {
            ...
            VanillaMagicAPI.LOGGER.log(Level.ERROR,
                    "Couldn't register item: " + item.toString() + ", in mapping: " + mappingName);
            e.printStackTrace();
        }
    }
     ...
     * In VanillaMagic mapping will start with "_" so "pickaxe" will be ->
     * "_pickaxe". <br>
     * It was made this way to prevent "pickaxe" and "axe" being counted as one
     * mapping.
     */
