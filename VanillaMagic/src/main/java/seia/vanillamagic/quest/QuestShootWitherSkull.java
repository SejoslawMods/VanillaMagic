...
{
	private final ItemStack shouldHaveLeftHand = ItemStackHelper.getHead(1, 1);
		...
		if(ItemStack.areItemsEqual(leftHand, shouldHaveLeftHand))
		{
			...
			if(ItemStack.areItemsEqual(rightHand, shouldHaveRightHand))
			{
				...
				if(player.hasAchievement(achievement))
				{
					...
					ItemStackHelper.decreaseStackSize(offHand, 1);
					if(ItemStackHelper.getStackSize(offHand) <= 0)
					{
