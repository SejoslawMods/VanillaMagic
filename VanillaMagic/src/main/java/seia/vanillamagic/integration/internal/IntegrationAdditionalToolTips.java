...
public class IntegrationAdditionalToolTips implements IIntegration
{
	...
	{
		if (!VMConfig.SHOW_ADDITIONAL_TOOLTIPS) return;
		...
		if (stack.getItem().isDamageable()) event.getToolTip().add(getDurabilityString(stack));
	}

	private String getHungerString(int hunger)
	{
		String ret = TextFormatting.GRAY + "Hunger: ";
		for (int i = 0; i < (hunger / 2); i++) ret += TextFormatting.DARK_RED + _symbol;
		if (hunger % 2 != 0) ret += TextFormatting.RED + _symbol;
		return ret;
	}
	...
	private String getSaturationString(float saturation)
	{
		...
		if (saturation <= 1) return ret += TextFormatting.DARK_RED + _symbol;
		if (saturation <= 2) return ret += TextFormatting.RED + _symbol;
		if (saturation <= 3) return ret += TextFormatting.GOLD + _symbol;
		if (saturation <= 4) return ret += TextFormatting.YELLOW + _symbol;
		if (saturation <= 5) return ret += TextFormatting.GREEN + _symbol;
		if (saturation <= 6) return ret += TextFormatting.DARK_GREEN + _symbol;
		if (saturation <= 7) return ret += TextFormatting.DARK_AQUA + _symbol;
		if (saturation <= 8) return ret += TextFormatting.BLUE + _symbol;
		if (saturation <= 9) return ret += TextFormatting.DARK_PURPLE + _symbol;
		else return ret += TextFormatting.LIGHT_PURPLE + _symbol;
	}
