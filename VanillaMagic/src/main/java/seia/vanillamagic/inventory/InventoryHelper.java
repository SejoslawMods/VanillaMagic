...
public class InventoryHelper 
{
	...
	public static boolean hasInventoryFreeSpace(IInventory inventoryIn, EnumFacing side)
	{
		if (inventoryIn instanceof ISidedInventory)
		{
			...
			{
				ItemStack stack = iSidedInventory.getStackInSlot(slot);
				if (ItemStackUtil.isNullStack(stack)) return true;
			}
		}
		else
		{
	...
	/**
	 * Pulls from the specif ied slot in the inventoryIn and places in any available slot in the inventoryOut. 
	 * 
	...
	/**
	 * Can insert the specif ied item from the specif ied slot on the specif ied side?
	 */
	...
	/**
	 * Can extract the specif ied item from the specif ied slot on the specif ied side?
	 */
	...
	/**
	 * @return Returns the IInventory (if  applicable) of the TileEntity at the specif ied position.
	 */
	@Nullable
			...
			if (!ItemStackUtil.isNullStack(checkingStack))
			{
				...
				if (blockFromItem != null)
				{
					...
					if (blockFromItem.hasTileEntity(blockFromItemState))
					{
						...
						if (tileFromBlock instanceof IInventory) return i;
					}
				}
			}
		}
