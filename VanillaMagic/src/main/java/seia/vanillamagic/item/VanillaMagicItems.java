...
public class VanillaMagicItems
{
	...
	{
		for (ICustomItem customItem : CUSTOM_ITEMS) customItem.registerRecipe();
	...
	{
		if (ItemStackUtil.isNullStack(checkingStack) || customItem == null) return false;
	...
	{
		if (ItemStackUtil.isNullStack(checkingStack) || customBucket == null) return false;
	...
	{
		for (ICustomItem ci : CUSTOM_ITEMS) list.add(ci.getItem());
		for (IEnchantedBucket eb : ENCHANTED_BUCKETS) list.add(eb.getItem());
		for (IPotionedCrystal pc : POTIONED_CRYSTALS) list.add(pc.getItem());
		return list;
	}
}